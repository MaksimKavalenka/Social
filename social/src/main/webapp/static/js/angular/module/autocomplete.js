  ...
  return {
      ...
      '<div class="ac-container" ' +
        ...
        '<ul class="ac-menu"> ' +
          '<li ng-repeat="result in results" ng-if="$index > 0" ' +
            ...
            '<a class="ac-user" ng-click="add(result.model)" ng-bind-html="result.value"></a>' +
          '</li>' +
        '</ul>' +
    ...
    controller: ['$rootScope', '$scope', function($rootScope, $scope) {
      ...
      $scope.add = function(model) {
    	var index = $rootScope.models.indexOf(model);
    	if (index >= 0) {
    	  $rootScope.models.splice(index, 1);
    	}
    	$rootScope.users.push(model);
      }
